...
export default function SavedDrawer({ isOpen, onClose, saved, onUnsave }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', backgroundColor: '#191e24', width: '100%', maxWidth: 380, height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 40px #00000060', borderLeft: '1px solid #2a323c' }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottom: '1px solid #2a323c' }}>
          <div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 20, color: '#fff', margin: 0 }}>❤️ Wishlist</h2>
            <p style={{ color: '#6b7280', fontSize: 13, margin: 0 }}>{saved.length} professors saved</p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: 18 }}>
            <FaTimes />
          </button>
        </div>
