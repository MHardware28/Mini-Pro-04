...
function Bar({ value, max = 5, color }) {
  return (
    <div style={{ flex: 1, backgroundColor: '#2a323c', borderRadius: 999, height: 6 }}>
...
export default function ProfCard({ prof, isSaved, onViewDetail }) {
  const ratingColor = prof.rating >= 4.5 ? '#36d399' : prof.rating >= 3.5 ? '#570df8' : prof.rating >= 2.5 ? '#fbbd23' : '#f87272'

  return (
    <div style={{ backgroundColor: '#191e24', border: '1px solid #2a323c', borderRadius: 24, width: '100%', height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px #00000060' }}>
      {/* Image */}
      <div style={{ position: 'relative' }}>
        <img src={prof.avatar} alt={prof.name} style={{ width: '100%', height: 200, objectFit: 'cover' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, #191e24, transparent)' }} />
        {isSaved && (
          <span style={{ position: 'absolute', top: 12, right: 12, backgroundColor: '#570df8', color: '#fff', borderRadius: 999, padding: '3px 10px', fontSize: 12, fontWeight: 600 }}>
            ❤️ Saved
          </span>
        )}
        <span style={{ position: 'absolute', top: 12, left: 12, backgroundColor: '#1d232a99', color: '#a6adbb', borderRadius: 999, padding: '3px 10px', fontSize: 12 }}>
          {prof.year}
        </span>
      </div>
      ...
      <div style={{ padding: 16, flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          ...
          <p style={{ color: '#570df8', fontWeight: 500, fontSize: 14, margin: '2px 0' }}>{prof.subject}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div className="flex items-center gap-2">
            ...
            <Bar value={prof.rating} color="#570df8" />
